...
import { LayoutDashboard, CreditCard, BarChart3, Target, Settings, Zap, LogOut, Users } from 'lucide-react'
...
export function Sidebar() {
  ...
  function NavLink({ href, label, icon: Icon }: { href: string; label: string; icon: React.ElementType }) {
    ...
    return (
      <Link href={href} className={clsx(
        ...
        active ? 'bg-violet/10 text-violet-light border border-violet/20' : 'text-ink-3 hover:text-ink-2 hover:bg-[#14142a]'
      )}>
  ...
  return (
    <aside className="w-[220px] min-w-[220px] h-screen bg-canvas border-r border-rim flex flex-col relative overflow-hidden">
